...
namespace Atlas
{
    public class CommandHandler
    {
        ...
        private static void FX(Entity sender, string[] args)
        {
            ...
            {
                sender.Call("iprintlnbold", AtlasiSnipe.ScriptConfiguration.TryGetFeedbackMessage("cmdFXDisabled"));
                return;
            }
            ...
            if (args.Length == 1)
            {
                if (args[0].ToLower().Equals("on") || args[0].Equals("1"))
                {
                    ...
                    sender.Call("iprintlnbold", "FX are now enabled.");
                }
                else if (args[0].ToLower().Equals("off") || args[0].Equals("0"))
                {
                    ...
                    sender.Call("iprintlnbold", "FX are now disabled.");
                }
                else
                {
                    sender.Call("iprintlnbold", "Syntax: !fx <on/off/1/0>");
                }
            }
            else
            {
        ...
        private static void FilmTweak(Entity sender, string[] args)
        {
            ...
            {
                sender.Call("iprintlnbold", AtlasiSnipe.ScriptConfiguration.TryGetFeedbackMessage("cmdFilmTweakDisabled"));
                return;
            }
            ...
            switch (args[0])
            {
                ...
                case "25":
                    ...
                    break;
                case "night":
                    sender.Call("visionsetnightforplayer", "paris_ac130_night");
                    break;
                case "reset":
                default:
                    sender.Call("visionsetnakedforplayer", Function.Call<string>("mapname"));
            ...
            sender.Call("iprintlnbold", string.Format("Film Tweak {0} applied.", args[0]));
        }
        ...
        private static void GiveAmmo(Entity sender, string[] args)
        {
            ...
            {
                sender.Call("iprintlnbold", AtlasiSnipe.ScriptConfiguration.TryGetFeedbackMessage("cmdGiveAmmoDisabled"));
                return;
            }

            if (args.Length != 0)
            {
                sender.Call("iprintlnbold", "Syntax: !ga");
                return;
            }

            sender.Call("givemaxammo", sender.CurrentWeapon);
            sender.Call("iprintlnbold", "You have been given maximum ammo for " + sender.CurrentWeapon);
        }
    }
}
